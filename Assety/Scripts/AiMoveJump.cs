...
public class AiMoveJump : MonoBehaviour {
    ...
    /*void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(50, 25, 50, 30), "Click")) //Tworzenie guzika Click na scenie do wykoania ruchu statkiem
        {
            click = true;
    ...
    {
        //currentMovePoint = 3;
        currentMovePoint = GetComponent<ShipStats>().movePoints;
        StartCoroutine(RangeAndMove());
    }

    IEnumerator RangeAndMove()
    {
        //animationShip.GetComponent<AnimationShip>().StartTeleport(transform.position); // Animacja teleportacji
        moveRange = currentMovePoint;
    ...
    void Move()
    {
        ...
        foreach (GameObject hex in GameObject.FindGameObjectsWithTag("HexMap"))
        {
                ...
                Debug.Log("Hex znaleziony" + moveHex.GetComponent<HexNumber>().hex_x + " " + moveHex.GetComponent<HexNumber>().hex_y);
                    ...
                    if (moveHex.GetComponent<HexNumber>().occupied == true && (moveHex.GetComponent<HexNumber>().hex_x != currentPos_x || moveHex.GetComponent<HexNumber>().hex_y != currentPos_y))
                    {
                        ...
                        if (moveIsPossible == true)
                        {
                            ...
                            {
                            animationShip.GetComponent<AnimationShip>().StartTeleport(transform.position);
                            transform.position = new Vector3(fixedMoveHex.transform.position.x, fixedMoveHex.transform.position.y, -2);
                                ...
                                currentMovePoint -= dist; //odejmij punkt purchu
                                movePos_x = fixedMovePos_x;
                                movePos_y = fixedMovePos_y;
                            animationShip.GetComponent<AnimationShip>().EndtTeleport(transform.position);
            ...
            //animationShip.GetComponent<AnimationShip>().EndtTeleport(transform.position);
            GetComponent<ShipStats>().movePoints = currentMovePoint;
        }
    ...
    void FindMove()
    {
        bool doMove = true;
        dist = 0;
        for (int k = 0; k < currentMovePoint; k++)
        {
            if (movePos_x == targetPos_x && movePos_y == targetPos_y && dist == 0)
            {
                doMove = false;
                break;

            }
            else if (movePos_x == targetPos_x && movePos_y == targetPos_y)
            {
                doMove = true;
                break;
            }
            ...
            if (movePos_y % 2 == 0)
            {
                if (movePos_y < targetPos_y)
                {
                    if (movePos_x < targetPos_x)
                    {
                        movePos_y++;
                        i = 0;
                    }
                    else if (movePos_x == targetPos_x)
                    {
                        movePos_y++;
                        i = 5;
                    }
